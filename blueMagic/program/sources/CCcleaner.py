...
# Internal filename: 'CCcleaner.py'
...
def close_browsers(log=None):
    ...
    for p in processes:
        ...
        if log:
            log('[INFO] Closed {p} if it was running')
...
def clean_browser_sessions(log=None):
    ...
    for name, base in BROWSERS.items():
        if name in ['Chrome', 'Edge', 'Brave']:
            ...
            for profile in base.glob('*'):
                freed += delete_path(profile / 'sessionstore.jsonlz4')
    return freed
def clean_browser_max(log=None):
    ...
    for name, base in BROWSERS.items():
        if not base.exists():
            continue
        if name in ['Chrome', 'Edge', 'Brave']:
            for d in CACHE_DIRS + SITE_DATA_DIRS:
                freed += delete_path(base / d)
            for f in SESSION_FILES:
                freed += delete_path(base / f)
            freed += delete_path(base / 'History')
        else:
            freed += clean_folder(str(base), log=log)
    return freed
