...
class IDCardCreator:
    def __init__(self, root):
        self.root = root
        self.root.title('ud83eudeaa ID Card Fraud Master')
        ...
        title = ctk.CTkLabel(main_frame, text='ud83eudeaa ID Card Fraud Master', font=ctk.CTkFont(family='Arial', size=24, weight='bold'), text_color='#ff3333')
        title.pack(pady=10)
        ...
        ctk.CTkButton(btn_frame, text='ud83eude84 Generate', command=self.generate_card, fg_color='#ff4500', text_color='white', font=ctk.CTkFont(size=12, weight='bold')).pack(fill='x', pady=5)
    def generate_card(self):
        ...
        for label, key in [('Name', 'name'), ('Gender', 'gender'), ('Age', 'age'), ('DOB', 'dob'), ('Blood', 'blood'), ('Mobile', 'mobile'), ('Address', 'address')]:
            ...
            y += spacing
        if self.use_qr.get() and self.qr_url.get().strip():
            qr_data = self.qr_url.get()
        else:
